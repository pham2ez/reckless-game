<template>
  <div class="coup">
    <div v-if="gameInfo !== null"> My Cards: {{myCards}}</div>
    <div v-if="gameInfo !== null">{{players[gameInfo.playerIndex]}}'s turn</div>
    ...
    <div class="cards" v-if="(currentPlayer || blockingPlayer) && !coup">
      <!-- only on their turn -->
      <Card v-bind:role="'D'"
    ...
    <div class="mine">
      <!-- The modals -->
        <b-modal :visible="showChoose" hide-footer hide-header>
        ...
        <b-modal v-if="currentAction !== null" :visible="show" hide-footer hide-header>
        ...
        <b-modal v-if="gameInfo !== null" :visible="showKill" ok-only hide-header hide-footer>
          <b-button v-for="i in gameInfo.myCards.length"
            v-bind:key="gameInfo.myCards[i-1] + i"
            :pressed.sync="inKill[i-1]"
            @click="kill(i-1)">
            {{dict[gameInfo.myCards[i-1]]}}</b-button>
            <b-button :disabled="numKill !== 0" @click="handleKill">Finish</b-button>
        </b-modal>
    </div>
  </div>
</template>
...
export default {
  ...
  created: function(){
    ...
    socket.on("challenge",(req)=>{
      if(this.username === req.toPlayer && this.gameInfo.myCards.includes(req.action)){
        let kill = this.chosenAction === "A"? 2: 1;
        socket.emit("challengeResults",{"loser": req.fromPlayer, "kill": kill});
        eventBus.$emit("add-message",{"message": this.username + " did have " + 
        this.dict[this.chosenAction], "roomID": this.roomID});
        if(this.chosenAction !== "A"){
          this.finish();
        }
    ...
    eventBus.$on("chosen-card", (req)=>{
      ...
      + this.dict[this.chosenAction], "roomID": this.roomID});
        socket.emit("action", {"action": this.chosenAction, "blockedAction": this.currentAction.action,
      ...
      eventBus.$emit("add-message",{"message": this.username + " chose " + this.dict[req], "roomID": this.roomID});
        socket.emit("action", {"action": this.chosenAction, "blockable": false,
  ...
  methods: {
    ...
    foreignAid: function(){
      ...
      socket.emit("action", {"action": this.chosenAction, "fromPlayer": this.username,
    ...
    choose: function(player){
      ...
      socket.emit("action", {"action": this.chosenAction, "blockable": this.chosenAction !== "COUP", 
    ...
    challenge: function(){
      ...
      socket.emit("challenge", {"action": this.currentAction.action,
      "fromPlayer": this.username, "toPlayer": this.currentAction.fromPlayer});
    },
    handleOk: function(){
      if(this.currentAction.blockedAction === undefined){
      eventBus.$emit("add-message", {"message": "Action successfully went through", "roomID": this.roomID});
      this.showBlock = false; this.showChallenge = false;
        if(this.currentAction.action === "N"){
          this.chosenAction = "N";
          this.numKill = 1;
          this.showKill = true; // for assassin
        }else{
          socket.emit("ok", {"player": this.currentAction.fromPlayer});
        }
      }else{
